...
type Config_structure struct {
	...
	Config       confiiiig_struct `json:"config"`
...
func Configgg(name, tag string) confiiiig_struct {
	sha_str := Manifest(name, tag)
	config := Manifest_shaw256(name, sha_str)
	if !strings.Contains(name, "/") {
		name = "library/" + name
	}
	m := Check_endpoint(name)
	token := Request_token(m)
	full_path := fmt.Sprintf("https://registry-1.docker.io/v2/%s/blobs/%s", name, config.Digest)
	req, err := http.NewRequest("GET", full_path, nil)
	if err != nil {
		panic(err)
	}
